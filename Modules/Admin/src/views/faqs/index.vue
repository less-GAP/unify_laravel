...
const tableConfig = {
  api: (params) => Api.get('faqs/list', { params }),
  addAction: (reload) => {
    router.push('/faqs/0')
  },
  itemActions: [
  ...
  columns: [
    ...
    { title: 'Name', key: 'name' },
    { title: 'Link SEO', key: 'slug' },
    { title: 'FAQs type', key: 'type' },
  ...
  selectionActions: [
    {
      title: 'Active',
      action(selectedKeys) {
        return Api.post('faqs/activeList', {
          'items': selectedKeys,
          'status': 'A'
        }).then(rs => {
          notification[rs.data.code == 0 ? 'error' : 'success']({
    ...
    <SectionMain>
      <DataTable v-bind="tableConfig">
        ...
        <template #cell[image]="{ item, column }">
          <img class="w-20 h-auto float-left" :src="item.image_url" :alt="item.name" />
        </template>
